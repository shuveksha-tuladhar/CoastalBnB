      ...
      <ul>
        ...
        <div id='num-guests'>
              ...
              <label>
                ...
                min="1" max={bedrooms + 1}  placeholder='1' />
              </label>
            </div>
      </ul>
      <br/>
          <ul className="fees">
            <span className="underline">
              ${price + (numGuests ? (numGuests-1)*10 : 0)} X {(startDate && endDate) ? daysDiff(startDate, endDate) : 0} nights
            </span>
            <span>
              {(startDate && endDate) ? '$'+ daysDiff(startDate, endDate) *(price + (numGuests ? (numGuests-1)*10 : 0)) : '$0'}
            </span>
            <span className="underline">Service fee</span>
            <span>{'$' + Math.floor(((startDate && endDate) ? daysDiff(startDate, endDate) : 0) * (price + (numGuests ? (numGuests-1)*10 : 0)) / 7)}</span>
            <div className='solid-line-reservations-2'></div>
            <br/>
            <span>Total cost</span>
            <span>
              {'$' + Math.floor(((startDate && endDate) ? daysDiff(startDate, endDate) : 0) * (price + (numGuests ? (numGuests-1)*10 : 0)) * 8 / 7 )}
            </span>
          </ul>
